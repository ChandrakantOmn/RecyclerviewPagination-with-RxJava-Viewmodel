...
class UserListViewModelTestNew{
    @get:Rule
    ...
    lateinit var mainViewModel: UserListViewModel


    @Before
    fun setUp() {
        MockitoAnnotations.initMocks(this)
        this.mainViewModel = UserListViewModel(this.userService)
    }
    ...
    fun fetchRepositories_positiveResponse() {
        ...
        this.mainViewModel.user!!.observeForever(observer)

        this.mainViewModel.loadMore(0)

        assertNotNull(this.mainViewModel.user!!.value)
        assertEquals(LiveDataResult.Status.ERROR, this.mainViewModel.user.value?.status)
